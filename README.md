...
- `FIREBOLT_TRANSPORT_LOG_FILE`: absolute path to append logs to (relative paths are rejected); if unset, logs go to stderr (or syslog when built with `ENABLE_SYSLOG`). Read on each log call, so changes take effect immediately without reconnect.
